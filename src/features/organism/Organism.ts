...
class Organism implements OrganismInterface {
  ...
  constructor(col: number, row: number, environment: OrganismEnvironment, parent?: Organism) {
    ...
    this.can_rotate = (this.environment === 'world')
      ? this.store.worldEnvironment.config.hyperparams.rotationEnabled
      : false;
    this.move_count = 0;
    this.move_range = 4;
  ...
  foodNeeded() {
    const extraMoverFoodCost = this.store.worldEnvironment.config.hyperparams.extraMoverFoodCost;
  ...
  lifespan() {
    const lifespanMultiplier = this.store.worldEnvironment.config.hyperparams.lifespanMultiplier;
  ...
  reproduce(grid_map: GridMap) {
    ...
    const {
      ...
      useGlobalMutability,
      globalMutability,
    } = this.store.worldEnvironment.config.hyperparams;
  ...
  isClear(grid_map: GridMap, col: number, row: number, rotation: number = this.rotation_direction) {
    const foodBlocksReproduction = this.store.worldEnvironment.config.hyperparams.foodBlocksReproduction;
